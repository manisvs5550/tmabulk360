...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        $ship_assigned = trim($_POST['ship_assigned'] ?? '') ?: null;
        ...
        if ($username === '' || $password === '') {
            ...
            if ($check->fetch()) {
                ...
                $stmt = $db->prepare('INSERT INTO users (username, password, ship_assigned) VALUES (?, ?, ?)');
                $stmt->execute([$username, $hash, $ship_assigned]);
            ...
            if ($check->fetch()) {
                ...
                if ($password !== '') {
                    if (strlen($password) < 6) {
                        ...
                        $hash = password_hash($password, PASSWORD_BCRYPT);
                        $stmt = $db->prepare('UPDATE users SET username = ?, password = ?, ship_assigned = ?, updated_at = NOW() WHERE id = ?');
                        $stmt->execute([$username, $hash, $ship_assigned, $id]);
                        $success = t('user_updated');
                    }
                } else {
                    $stmt = $db->prepare('UPDATE users SET username = ?, ship_assigned = ?, updated_at = NOW() WHERE id = ?');
                    $stmt->execute([$username, $ship_assigned, $id]);
                    $success = t('user_updated');
                }
            }
        }
...
// Fetch users
$users = $db->query('SELECT u.id, u.username, u.ship_assigned, u.created_at, u.updated_at FROM users u ORDER BY u.id')->fetchAll();
                            ...
                            <td>
                                <div class="action-btns">
                                    <button class="btn-icon btn-icon--edit" title="<?= e(t('edit')) ?>"
                                        onclick="openEditUser(<?= $user['id'] ?>, '<?= e(addslashes($user['username'])) ?>', '<?= e(addslashes($user['ship_assigned'] ?? '')) ?>')">
    ...
    <div class="modal-card">
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <select id="add-ship" name="ship_assigned" class="form-select">
    ...
    <div class="modal-card">
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <select id="edit-ship" name="ship_assigned" class="form-select">
                    <option value=""><?= e(t('none')) ?></option>
                    <?php foreach ($vessels as $v): ?>
                    <option value="<?= e($v['name']) ?>"><?= e($v['name']) ?></option>
                    <?php endforeach; ?>
...
function openEditUser(id, username, ship) {
